...
Character Array element After Sorting using the Selection sort in Descending Order :
q p c b a
...
#include<iostream>
using namespace std;
void sortTheArrayUsingTheSelectionSortInDescendingOrder(char arr[], int n) {
  for(int i=0; i<n; i++){
    int index = i;
    for(int j=i+1; j<n; j++) {
      if(arr[j] > arr[index]){
...
int main() {
  ...
  // function call
  sortTheArrayUsingTheSelectionSortInDescendingOrder(charArr, n);

  // Display the array element in Descending Order
  cout << "Character Array element After Sorting using the Selection sort in Descending Order : " << endl;
  for(int i=0; i<n; i++){
    cout << charArr[i] << " ";
  }
}
